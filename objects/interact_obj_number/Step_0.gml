if (global.accept_minigame_comsci==true && place_meeting(x, y, obj_number)) {
	sprite_index = Sprite3;
	image_index = 0;

	if(is_get_score){
		global.number2_GET = true
		is_get_score = false
	}
}else if(global.accept_minigame_comsci==true && !place_meeting(x, y, obj_number)){
	sprite_index = Red;
	image_index = 0;
